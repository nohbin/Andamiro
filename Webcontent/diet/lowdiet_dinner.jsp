...
<div class="container border mt-3 rounded-3 w-50">
		<ul class="nav nav-pills nav-fill row">
			...
			<li class="nav-item col-12 col-md-3"><a class="nav-link active" style="color: black;" aria-current="page" href='<c:url value="SubDietServlet?command=lowDiet"/>'>
				저칼로리식단
				</a>
			</li>
       ...
       <div style="height: 1rem;"></div>
	 			  ...
	 			  <table>
	 			  	<tr>
	 			  		...
	 			  		<c:forEach var="dietDetailfood" items="${dietDetailList1}">
							<div>
								<div>
									<div class="fw-bold" style="margin-left:10rem;">
		...
		<div class="container">
					...
					onclick="location.href='SubDietServlet?command=lowDiet'">아침 식단
					</button>
					<button type="button" class="btn btn-outline-success btn-sm"
					onclick="location.href='SubDietServlet?command=lowDietLunch'">점심 식단
					</button>
					<button type="button" class="btn btn-outline-success btn-sm"
					onclick="location.href='SubDietServlet?command=lowDietDinner'">저녁 식단
					</button>		
			<div class="row">
			...
			<table class="table table-hover" style="height: 300px;">
				...
				<tbody class="table-secondary">
					<c:forEach var="dietDetail6" items="${dietDetailList6}">
					<tr>
						<td class="h5 border-end">${dietDetail6.foodName}</td>
						<td class="h5 border-end">${dietDetail6.kcal}</td>
						<td class="h5">${dietDetail6.component}</td>
					</tr>
					</c:forEach>
				</tbody>
			</table>
